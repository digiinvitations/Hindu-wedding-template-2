...
export async function saveConfigToDb(newConfig: WeddingConfig) {
  ...
  try {
    console.log("[saveConfigToDb] Saving to Firestore (website/permanent_data)...");
    await setDoc(doc(db, "website", "permanent_data"), { data: newConfig });
    ...
    handleFirestoreError(err, OperationType.WRITE, "website/permanent_data");
  }
}
...
export async function fetchConfigFromDb(): Promise<WeddingConfig | null> {
  ...
  try {
    console.log("[fetchConfigFromDb] Fetching from Firestore (website/permanent_data)...");
    const docSnap = await getDoc(doc(db, "website", "permanent_data"));
    if (docSnap.exists()) {
      console.log("[fetchConfigFromDb] Document found in Firestore.");
      const serverConfig = docSnap.data().data as WeddingConfig;
      ...
      // Attempt to migrate from old settings/config if new one doesn't exist
      console.log("[fetchConfigFromDb] Checking old settings/config for migration...");
      const oldDocSnap = await getDoc(doc(db, "settings", "config"));
      if (oldDocSnap.exists()) {
        const oldConfig = oldDocSnap.data() as WeddingConfig;
        if (oldConfig && Object.keys(oldConfig).length > 0) {
          console.log("[fetchConfigFromDb] Migrating old config to new location...");
          await setDoc(doc(db, "website", "permanent_data"), { data: oldConfig });
          localStorage.setItem("wedding_config", JSON.stringify(oldConfig));
          window.dispatchEvent(new Event("wedding_config_updated"));
          return oldConfig;
        }
      }
